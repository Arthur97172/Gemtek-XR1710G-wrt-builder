...
| 发行版 | 默认 LAN IP | 默认密码 | 备注 |
|--------|-----------|---------|------|
| **iStoreOS** | `192.168.10.1` | `password` | LuCI + iStore 管理面板 |
| **OpenWrt 24.10** | `192.168.10.1` | 无密码（首次按 `Enter`，用 `passwd` 设新密码） | 标准 LuCI |
| **ImmortalWrt 24.10** | `192.168.10.1` | 无密码（首次按 `Enter`，用 `passwd` 设新密码） | 增强插件生态 |
